...
int cpu_exec(int n){
	...
	for(int i = -3; i < 2 * n; i++){
		if(ysyxSoCFull -> clock){
			...
			svScope scope;
			// scope = svGetScopeFromName("TOP.ysyxSoCFull.ysyx_25030077.g_mem");
			// svSetScope(scope);
			...
			if(wmask > 0 && valid){
				...
				scope = svGetScopeFromName("TOP.ysyxSoCFull.ysyx_25030077.h_data_control");
				svSetScope(scope);
			...
			if(FTRACE && ix % 8 == 7){
				uint32_t dnpc_data;
				scope = svGetScopeFromName("TOP.ysyxSoCFull.ysyx_25030077.j_pc_next");
				svSetScope(scope);
		...
		if (DIFFTEST && !(ysyxSoCFull -> clock) && !(ysyxSoCFull -> reset) && ix % 8 == 7){
			difftest_step();
		}
